...
def get_db_table_as_df(con, name, **kwargs):
...
def get_table_for_variable(variableID):
    table_name = 'datavalues'
    sql = """SELECT * FROM {} WHERE VariableID={};""".format(table_name, variableID)
    df = get_db_table_as_df(get_con(), table_name, sql=sql)
    return df